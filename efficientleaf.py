...
def mel_filter_params(
    ...
    and sigmas of a mel filter bank
...
class GroupedGaborFilterbank(nn.Module):
    ...
    def clamp_parameters(self):
        ...
        self.bandwidths.data = self.bandwidths.data.clamp(
            min=self.sigma_lower,
            max=self.sigma_upper
        )
...
class GaborFilterbank(nn.Module):
    """
    Torch module that functions as a gabor filterbank.
    Initializes n_filters center frequencies and bandwidths that are based on a
    mel filterbank. The parameters are used to calculate Gabor filters for a 1D
    convolution over the input signal. The squared modulus is taken from the
    results. To reduce the temporal resolution a gaussian lowpass filter is
    calculated from pooling_widths, which are used to perform a pooling
    operation. The center frequencies, bandwidths and pooling_widths are
